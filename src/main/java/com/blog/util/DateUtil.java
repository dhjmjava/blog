...
public class DateUtil {
	...
	//指定年月日+当前时间时分秒
	public static Date strToSpecialDate(String dateStr){
		if(null == dateStr){
			return new Date();
		}
		...
		try {
			...
			return strToDate(builder.toString());
	...
	public static Date currentDate(){
		...
		try {
			String currentDates = format.format(new Date());
			return format.parse(currentDates);
		} catch (ParseException e) {
			return new Date();
		}
	}
	...
	*/
	...
	public static String[] getDate(String dateStr){
		...
		Calendar c = Calendar.getInstance();
		c.set(Integer.valueOf(strings[0]),Integer.valueOf(strings[1])-1,1);
		int days = c.getActualMaximum(Calendar.DAY_OF_MONTH);
		String startTime = dateStr+ "-01 00:00:00";
		String endTime = dateStr + "-" +days+" 23:59:59";
	...
	/**
	 * 获取30天内的所有日期（字符串格式）
	 * @param 可以为空 格式日期加在日期前后缀
	 * @return
	 */
	public static List<String> getLastMonthAllDateStr(String format){
		...
		for (int i = 0; i < 30; i++) {
			...
			dateList.add(format+dateFormat.format(calendar.getTime())+format);
		}
		return dateList;
	}
}
